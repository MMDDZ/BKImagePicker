...
{
    if (!_editImageView) {
        _editImageView = [[UIImageView alloc]initWithFrame:CGRectMake(10, 30, BK_SCREENW - 20, BK_SCREENH - 140)];
...
-(void)rotationImageAction
{
    ...
    if (flag) {
        ...
        CGFloat editImage_wh_scale = _editImageView.image.size.width/_editImageView.image.size.height;
        if (scale > editImage_wh_scale) {
            scale = editImage_wh_scale;
        }
    }else{
